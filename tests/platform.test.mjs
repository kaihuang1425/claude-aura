...
test("Windows window clamp fits a synthetic 1280x720 working area", async () => {
  ...
  if (process.platform === "win32") {
    ...
    const powershell = [
      ...
      "  $requested=$form.Size",
      "  $expectedWidth=[Math]::Min($requested.Width,1256)",
      "  $expectedHeight=[Math]::Min($requested.Height,696)",
      "  $expected=[Drawing.Rectangle]::new([Math]::Max(12,[Math]::Floor((1280-$expectedWidth)/2)),[Math]::Max(12,[Math]::Floor((720-$expectedHeight)/2)),$expectedWidth,$expectedHeight)",
      "  Test-AuraUiFormWithinWorkingArea -Form $form -Margin 12 -SyntheticWorkingArea $area",
      "  if($form.Bounds -ne $expected){throw \"Large window clamped to unexpected bounds: $($form.Bounds), expected $expected from OS-accepted size $requested\"}",
...
test("Windows startup layout separates compact maximize from spacious saved bounds", async () => {
  ...
  if (process.platform === "win32") {
    const powershell = [
      ...
      "$normalBounds=$script:AuraNormalWindowSnapshot.bounds; $normalDpi=$script:AuraNormalWindowSnapshot.dpi",
      ...
      "if($script:AuraNormalWindowSnapshot.bounds -ne $normalBounds -or $script:AuraNormalWindowSnapshot.dpi -ne $normalDpi -or $normalDpi -ne 144){throw 'Maximized cross-DPI state replaced the paired normal snapshot'}",
      "$form.Dispose()",
