...
| Tests, parsing, visual cycle, preview build, and release exclusions | Sixteen checks in `tests/run-tests.mjs`, including executable Studio framing/cache regressions; PowerShell parse; `npm run verify:cycle` 17/17 with all sixteen light/dark renders inspected; results recorded in `docs/IMPLEMENTATION_REPORT.md`; `scripts/build-release.mjs` exclusion policy | Proven for current source checks |
...
console warnings or errors. HUMAN CHECKPOINT B remains pending user approval.
...
platform variance, and live install/uninstall.
